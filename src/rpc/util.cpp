...
std::vector<RPCResult> TxDoc(const std::string& txid_field_doc, TxDocOptions opts)
{
    ...
    vin_item.push_back({RPCResult::Type::NUM, "sequence", "The script sequence number"});
    ...
    if (opts.wallet) {
        vout_item.push_back({RPCResult::Type::BOOL, "ischange", /*optional=*/true, "Output script is change (only present if true)"});
    }
    ...
    if (opts.fee) {
        tx.push_back({RPCResult::Type::NUM, "fee", "transaction fee in " + CURRENCY_UNIT + ", omitted if block undo data is not available"});
    }

    if (opts.hex) {
        tx.push_back({RPCResult::Type::STR_HEX, "hex", "The hex-encoded transaction data"});
    }
